(module $LabSwatch
  ...
  (data (i32.const 285) "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1 1\" width=\"160\" height=\"160\" data-action ")
  (data (i32.const 381) "data-mousedown=\"l_changed\"")
  (data (i32.const 408) "data-mousedown=\"a_changed\"")
  (data (i32.const 435) "data-mousedown=\"b_changed\"")
  (data (i32.const 462) ">\n")
  (data (i32.const 466) "<defs>\n")
  (data (i32.const 475) "</defs>\n")
  (data (i32.const 485) "<rect width=\"1\" height=\"1\" fill=\"url('#lab-")
  (data (i32.const 529) "l")
  (data (i32.const 531) "a")
  (data (i32.const 533) "b")
  (data (i32.const 535) "-gradient')\" />\n")
  (data (i32.const 553) "</svg>\n")
  (data (i32.const 562) "<circle data-drag-knob=\"\" cx=\"")
  (data (i32.const 593) "\" cy=\"")
  (data (i32.const 600) "\" r=\"0.05\" fill=\"white\" stroke=\"black\" stroke-width=\"0.01\" />\n")
  (data (i32.const 664) "<linearGradient id=\"")
  (data (i32.const 685) "lab-l-gradient")
  (data (i32.const 700) "lab-a-gradient")
  (data (i32.const 715) "lab-b-gradient")
  (data (i32.const 730) "\" gradientTransform=\"scale(1.414) rotate(45)\">\n")
  (data (i32.const 779) "</linearGradient>\n")
  (data (i32.const 799) "<stop offset=\"")
  (data (i32.const 814) "%\" stop-color=\"")
  (data (i32.const 830) "\" />\n")
  (data (i32.const 837) "rgba(")
  (data (i32.const 843) ",")
  (data (i32.const 845) ",1)")
  ...
  (func $mousedown_offset (export "mousedown_offset") (param $x f32) (param $y f32)
    ...
    (local.get $x)
    (global.set $mouse_offset_x)
    (local.get $y)
    (global.set $mouse_offset_y)
    (f32.mul (f32.div (f32.add (f32.div (local.get $x) (global.get $swatch_size)) (f32.div (local.get $y) (global.get $swatch_size))) (f32.const 2.0)) (f32.const 100.0))
    (global.set $l)
    (call $log_f32 (global.get $l))
  )
  ...
  (func $swatch_svg (export "swatch_svg") (param $component_id i32) (result i32)
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 381))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 408))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 435))
      )
    )
    (call $bump_write_str (i32.const 462))
    (call $bump_write_str (i32.const 466))
    (call $do_linear_gradient (local.get $component_id))
    drop
    (call $bump_write_str (i32.const 475))
    (call $bump_write_str (i32.const 485))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 529))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 531))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 533))
      )
    )
    (call $bump_write_str (i32.const 535))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (call $bump_write_str (i32.const 553))
    (call $bump_write_done)
  )
  (func $do_drag_knob (param $offset f32) (result i32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 562))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 593))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 600))
    (call $bump_write_done)
  )
  ...
  (func $do_linear_gradient (param $component_id i32) (result i32)
    (local $i f32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 664))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 685))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 715))
      )
    )
    (call $bump_write_str (i32.const 730))
    (loop $Stops
    ...
    (call $bump_write_str (i32.const 779))
    (call $bump_write_done)
  )
  ...
  (func $do_css_color_lab_srgb (param $l f32) (param $a f32) (param $b f32) (result i32)
    ...
    (local.set $blue)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 837))
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 845))
    (call $bump_write_done)
  )
)
