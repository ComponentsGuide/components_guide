(module $LabSwatch
  ...
  (data (i32.const 285) "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1 1\" width=\"160\" height=\"160\" data-action data-mousedown-mousemove ")
  (data (i32.const 406) "data-mousedown=\"l_changed\"")
  (data (i32.const 433) "data-mousedown=\"a_changed\"")
  (data (i32.const 460) "data-mousedown=\"b_changed\"")
  (data (i32.const 487) ">\n")
  (data (i32.const 491) "<defs>\n")
  (data (i32.const 500) "</defs>\n")
  (data (i32.const 510) "<rect width=\"1\" height=\"1\" fill=\"url('#lab-")
  (data (i32.const 554) "l")
  (data (i32.const 556) "a")
  (data (i32.const 558) "b")
  (data (i32.const 560) "-gradient')\" />\n")
  (data (i32.const 578) "</svg>\n")
  (data (i32.const 587) "<circle data-drag-knob=\"\" cx=\"")
  (data (i32.const 618) "\" cy=\"")
  (data (i32.const 625) "\" r=\"0.05\" fill=\"white\" stroke=\"black\" stroke-width=\"0.01\" />\n")
  (data (i32.const 689) "<linearGradient id=\"")
  (data (i32.const 710) "lab-l-gradient")
  (data (i32.const 725) "lab-a-gradient")
  (data (i32.const 740) "lab-b-gradient")
  (data (i32.const 755) "\" gradientTransform=\"scale(1.414) rotate(45)\">\n")
  (data (i32.const 804) "</linearGradient>\n")
  (data (i32.const 824) "<stop offset=\"")
  (data (i32.const 839) "%\" stop-color=\"")
  (data (i32.const 855) "\" />\n")
  (data (i32.const 862) "rgba(")
  (data (i32.const 868) ",")
  (data (i32.const 870) ",1)")
  ...
  (func $swatch_svg (export "swatch_svg") (param $component_id i32) (result i32)
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 406))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 433))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 460))
      )
    )
    (call $bump_write_str (i32.const 487))
    (call $bump_write_str (i32.const 491))
    (call $do_linear_gradient (local.get $component_id))
    drop
    (call $bump_write_str (i32.const 500))
    (call $bump_write_str (i32.const 510))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 554))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 556))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 558))
      )
    )
    (call $bump_write_str (i32.const 560))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (call $bump_write_str (i32.const 578))
    (call $bump_write_done)
  )
  (func $do_drag_knob (param $offset f32) (result i32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 587))
  ...
  (func $do_linear_gradient (param $component_id i32) (result i32)
    (local $i f32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 689))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 725))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 740))
      )
    )
    (call $bump_write_str (i32.const 755))
    (loop $Stops
    ...
    (call $bump_write_str (i32.const 804))
    (call $bump_write_done)
  )
  ...
  (func $do_linear_gradient_stop (param $fraction f32) (param $l f32) (param $a f32) (param $b f32) (result i32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 824))
    ...
    drop
    (call $bump_write_str (i32.const 855))
    (call $bump_write_done)
  )
  (func $do_css_color_lab_srgb (param $l f32) (param $a f32) (param $b f32) (result i32)
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 868))
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 870))
    (call $bump_write_done)
  )
)
