(module $LabSwatch
  ...
  (data (i32.const 362) "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1 1\" width=\"160\" height=\"160\" class=\"touch-none\" data-action ")
  (data (i32.const 477) "data-pointerdown=\"l_changed\" data-pointerdown+pointermove=\"l_changed\"")
  (data (i32.const 547) "data-pointerdown=\"a_changed\" data-pointerdown+pointermove=\"a_changed\"")
  (data (i32.const 617) "data-pointerdown=\"b_changed\" data-pointerdown+pointermove=\"b_changed\"")
  (data (i32.const 687) ">\n")
  (data (i32.const 691) "<defs>\n")
  (data (i32.const 700) "</defs>\n")
  (data (i32.const 710) "<rect width=\"1\" height=\"1\" fill=\"url('#lab-")
  (data (i32.const 754) "l")
  (data (i32.const 756) "a")
  (data (i32.const 758) "b")
  (data (i32.const 760) "-gradient')\" />\n")
  (data (i32.const 778) "</svg>\n")
  (data (i32.const 787) "<circle data-drag-knob=\"\" cx=\"")
  (data (i32.const 818) "\" cy=\"")
  (data (i32.const 825) "\" r=\"0.05\" fill=\"white\" stroke=\"black\" stroke-width=\"0.01\" />\n")
  (data (i32.const 889) "<linearGradient id=\"")
  (data (i32.const 910) "lab-l-gradient")
  (data (i32.const 925) "lab-a-gradient")
  (data (i32.const 940) "lab-b-gradient")
  (data (i32.const 955) "\" gradientTransform=\"scale(1.414) rotate(45)\">\n")
  (data (i32.const 1004) "</linearGradient>\n")
  (data (i32.const 1024) "<stop offset=\"")
  (data (i32.const 1039) "%\" stop-color=\"")
  (data (i32.const 1055) "\" />\n")
  (data (i32.const 1062) "rgba(")
  (data (i32.const 1068) ",")
  (data (i32.const 1070) ",1)")
  ...
  (func $pointer_offset_changed (param $x f32) (param $y f32)
    (local $offset f32)
  ...
  (func $pointerdown_offset (export "pointerdown_offset") (param $x f32) (param $y f32)
    (call $pointer_offset_changed (local.get $x) (local.get $y))
  )
  (func $pointermove_offset (export "pointermove_offset") (param $x f32) (param $y f32)
    (call $pointer_offset_changed (local.get $x) (local.get $y))
  )
  ...
  (func $swatch_svg (export "swatch_svg") (param $component_id i32) (result i32)
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 477))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 547))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 617))
      )
    )
    (call $bump_write_str (i32.const 687))
    (call $bump_write_str (i32.const 691))
    (call $do_linear_gradient (local.get $component_id))
    drop
    (call $bump_write_str (i32.const 700))
    (call $bump_write_str (i32.const 710))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 754))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 756))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
    ...
    (call $bump_write_str (i32.const 760))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (call $bump_write_str (i32.const 778))
    (call $bump_write_done)
  )
  (func $do_drag_knob (param $offset f32) (result i32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 787))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 818))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 825))
    (call $bump_write_done)
  )
  ...
  (func $do_linear_gradient (param $component_id i32) (result i32)
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 925))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 940))
      )
    )
    (call $bump_write_str (i32.const 955))
    (loop $Stops
  ...
  (func $do_css_color_lab_srgb (param $l f32) (param $a f32) (param $b f32) (result i32)
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 1068))
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 1070))
    (call $bump_write_done)
  )
)
