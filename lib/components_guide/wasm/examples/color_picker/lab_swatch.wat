(module $LabSwatch
  ...
  (data (i32.const 375) "<svg viewBox=\"0 0 1 1\" width=\"1rem\" height=\"1rem\" fill=\"red\"><rect width=\"1\" height=\"1\" fill=\"")
  (data (i32.const 470) "\" /></svg> ")
  (data (i32.const 482) "</p>\n")
  (data (i32.const 489) "</output>\n")
  (data (i32.const 501) "lab(")
  (data (i32.const 506) "% ")
  (data (i32.const 509) " ")
  (data (i32.const 511) ")")
  (data (i32.const 513) "rgb(")
  (data (i32.const 518) "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1 1\" width=\"")
  (data (i32.const 584) "\" height=\"")
  (data (i32.const 595) "\" class=\"touch-none\" data-action ")
  (data (i32.const 629) "data-pointerdown=\"l_changed\" data-pointerdown+pointermove=\"l_changed\"")
  (data (i32.const 699) "data-pointerdown=\"a_changed\" data-pointerdown+pointermove=\"a_changed\"")
  (data (i32.const 769) "data-pointerdown=\"b_changed\" data-pointerdown+pointermove=\"b_changed\"")
  (data (i32.const 839) ">\n")
  (data (i32.const 843) "<defs>\n")
  (data (i32.const 852) "</defs>\n")
  (data (i32.const 862) "<rect width=\"1\" height=\"1\" fill=\"url('#lab-")
  (data (i32.const 906) "l")
  (data (i32.const 908) "a")
  (data (i32.const 910) "b")
  (data (i32.const 912) "-gradient')\" />\n")
  (data (i32.const 930) "</svg>\n")
  (data (i32.const 939) "<circle data-drag-knob=\"\" cx=\"")
  (data (i32.const 970) "\" cy=\"")
  (data (i32.const 977) "\" r=\"0.05\" fill=\"white\" stroke=\"black\" stroke-width=\"0.01\" />\n")
  (data (i32.const 1041) "<linearGradient id=\"")
  (data (i32.const 1062) "lab-l-gradient")
  (data (i32.const 1077) "lab-a-gradient")
  (data (i32.const 1092) "lab-b-gradient")
  (data (i32.const 1107) "\" gradientTransform=\"scale(1.414) rotate(45)\">\n")
  (data (i32.const 1156) "</linearGradient>\n")
  (data (i32.const 1176) "<stop offset=\"")
  (data (i32.const 1191) "%\" stop-color=\"")
  (data (i32.const 1207) "\" />\n")
  (data (i32.const 1214) "rgba(")
  (data (i32.const 1220) ",")
  (data (i32.const 1222) ",1)")
  ...
  (func $do_output_code (result i32)
    ...
    (call $do_css_lab)
    drop
    (call $bump_write_str (i32.const 470))
    (call $do_css_lab)
    drop
    (call $bump_write_str (i32.const 482))
    (call $bump_write_str (i32.const 339))
    (call $bump_write_str (i32.const 375))
    (call $do_css_rgb)
    drop
    ...
    drop
    (call $bump_write_str (i32.const 482))
    (call $bump_write_str (i32.const 489))
    (call $bump_write_done)
  )
  ...
  (func $do_css_rgb (export "do_css_rgb") (result i32)
    ...
    (local.set $r)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 513))
  ...
  (func $swatch_svg (export "swatch_svg") (param $component_id i32) (result i32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 518))
    ...
    (i32.add (call $format_f32 (global.get $swatch_size) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 595))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 629))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 699))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 769))
      )
    )
    (call $bump_write_str (i32.const 839))
    (call $bump_write_str (i32.const 843))
    (call $do_linear_gradient (local.get $component_id))
    drop
    (call $bump_write_str (i32.const 852))
    (call $bump_write_str (i32.const 862))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 906))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 908))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 910))
      )
    )
    (call $bump_write_str (i32.const 912))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (call $bump_write_str (i32.const 930))
    (call $bump_write_done)
  )
  (func $do_drag_knob (param $offset f32) (result i32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 939))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 970))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 977))
    (call $bump_write_done)
  )
  ...
  (func $do_linear_gradient (param $component_id i32) (result i32)
    (local $i f32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 1041))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 1062))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 1077))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 1092))
      )
    )
    (call $bump_write_str (i32.const 1107))
    (loop $Stops
    ...
    (call $bump_write_str (i32.const 1156))
    (call $bump_write_done)
  )
  ...
  (func $do_css_color_lab_srgb (param $l f32) (param $a f32) (param $b f32) (result i32)
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 1222))
    (call $bump_write_done)
  )
)
