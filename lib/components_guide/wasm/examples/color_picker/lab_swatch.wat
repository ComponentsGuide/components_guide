(module $LabSwatch
  ...
  (data (i32.const 362) "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1 1\" width=\"")
  (data (i32.const 428) "\" height=\"")
  (data (i32.const 439) "\" class=\"touch-none\" data-action ")
  (data (i32.const 473) "data-pointerdown=\"l_changed\" data-pointerdown+pointermove=\"l_changed\"")
  (data (i32.const 543) "data-pointerdown=\"a_changed\" data-pointerdown+pointermove=\"a_changed\"")
  (data (i32.const 613) "data-pointerdown=\"b_changed\" data-pointerdown+pointermove=\"b_changed\"")
  (data (i32.const 683) ">\n")
  (data (i32.const 687) "<defs>\n")
  (data (i32.const 696) "</defs>\n")
  (data (i32.const 706) "<rect width=\"1\" height=\"1\" fill=\"url('#lab-")
  (data (i32.const 750) "l")
  (data (i32.const 752) "a")
  (data (i32.const 754) "b")
  (data (i32.const 756) "-gradient')\" />\n")
  (data (i32.const 774) "</svg>\n")
  (data (i32.const 783) "<circle data-drag-knob=\"\" cx=\"")
  (data (i32.const 814) "\" cy=\"")
  (data (i32.const 821) "\" r=\"0.05\" fill=\"white\" stroke=\"black\" stroke-width=\"0.01\" />\n")
  (data (i32.const 885) "<linearGradient id=\"")
  (data (i32.const 906) "lab-l-gradient")
  (data (i32.const 921) "lab-a-gradient")
  (data (i32.const 936) "lab-b-gradient")
  (data (i32.const 951) "\" gradientTransform=\"scale(1.414) rotate(45)\">\n")
  (data (i32.const 1000) "</linearGradient>\n")
  (data (i32.const 1020) "<stop offset=\"")
  (data (i32.const 1035) "%\" stop-color=\"")
  (data (i32.const 1051) "\" />\n")
  (data (i32.const 1058) "rgba(")
  (data (i32.const 1064) ",")
  (data (i32.const 1066) ",1)")
  ...
  (func $swatch_svg (export "swatch_svg") (param $component_id i32) (result i32)
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 428))
    (i32.add (call $format_f32 (global.get $swatch_size) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 439))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 473))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 543))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 613))
      )
    )
    (call $bump_write_str (i32.const 683))
    (call $bump_write_str (i32.const 687))
    (call $do_linear_gradient (local.get $component_id))
    drop
    (call $bump_write_str (i32.const 696))
    (call $bump_write_str (i32.const 706))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
  ...
  (func $do_linear_gradient (param $component_id i32) (result i32)
    (local $i f32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 885))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 921))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 936))
      )
    )
    (call $bump_write_str (i32.const 951))
    (loop $Stops
    ...
    (call $bump_write_str (i32.const 1000))
    (call $bump_write_done)
  )
