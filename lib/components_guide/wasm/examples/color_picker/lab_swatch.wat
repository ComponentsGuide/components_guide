(module $LabSwatch
  ...
  (global $l (export "l") (mut f32) (f32.const 50.0))
  (global $a (export "a") (mut f32) (f32.const 100))
  (global $b (export "b") (mut f32) (f32.const -128))
  ...
  (data (i32.const 255) "<div class=\"flex\">\n")
  (data (i32.const 276) "</div>\n")
  (data (i32.const 285) "<output class=\"flex\">\n")
  (data (i32.const 309) "lab(")
  (data (i32.const 314) "% ")
  (data (i32.const 317) " ")
  (data (i32.const 319) ")")
  (data (i32.const 321) "</output>\n")
  (data (i32.const 333) "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1 1\" width=\"160\" height=\"160\" data-action ")
  (data (i32.const 429) "data-mousedown=\"l_changed\" data-mousedown-mousemove=\"l_changed\"")
  (data (i32.const 493) "data-mousedown=\"a_changed\" data-mousedown-mousemove=\"a_changed\"")
  (data (i32.const 557) "data-mousedown=\"b_changed\" data-mousedown-mousemove=\"b_changed\"")
  (data (i32.const 621) ">\n")
  (data (i32.const 625) "<defs>\n")
  (data (i32.const 634) "</defs>\n")
  (data (i32.const 644) "<rect width=\"1\" height=\"1\" fill=\"url('#lab-")
  (data (i32.const 688) "l")
  (data (i32.const 690) "a")
  (data (i32.const 692) "b")
  (data (i32.const 694) "-gradient')\" />\n")
  (data (i32.const 712) "</svg>\n")
  (data (i32.const 721) "<circle data-drag-knob=\"\" cx=\"")
  (data (i32.const 752) "\" cy=\"")
  (data (i32.const 759) "\" r=\"0.05\" fill=\"white\" stroke=\"black\" stroke-width=\"0.01\" />\n")
  (data (i32.const 823) "<linearGradient id=\"")
  (data (i32.const 844) "lab-l-gradient")
  (data (i32.const 859) "lab-a-gradient")
  (data (i32.const 874) "lab-b-gradient")
  (data (i32.const 889) "\" gradientTransform=\"scale(1.414) rotate(45)\">\n")
  (data (i32.const 938) "</linearGradient>\n")
  (data (i32.const 958) "<stop offset=\"")
  (data (i32.const 973) "%\" stop-color=\"")
  (data (i32.const 989) "\" />\n")
  (data (i32.const 996) "rgba(")
  (data (i32.const 1002) ",")
  (data (i32.const 1004) ",1)")
  ...
  (func $swatch_svg (export "swatch_svg") (param $component_id i32) (result i32)
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 429))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 493))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 557))
      )
    )
    (call $bump_write_str (i32.const 621))
    (call $bump_write_str (i32.const 625))
    (call $do_linear_gradient (local.get $component_id))
    drop
    (call $bump_write_str (i32.const 634))
    (call $bump_write_str (i32.const 644))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (if 
      (then
        (call $bump_write_str (i32.const 690))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 692))
      )
    )
    (call $bump_write_str (i32.const 694))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
    ...
    (call $bump_write_str (i32.const 712))
    (call $bump_write_done)
  )
  (func $do_drag_knob (param $offset f32) (result i32)
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 752))
    (i32.add (call $format_f32 (local.get $offset) (global.get $bump_offset)) (global.get $bump_offset))
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 759))
    (call $bump_write_done)
  )
  ...
  (func $do_linear_gradient (param $component_id i32) (result i32)
    (local $i f32)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 823))
    (i32.eq (local.get $component_id) (global.get $component_l))
    (if 
      (then
        (call $bump_write_str (i32.const 844))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_a))
    (if 
      (then
        (call $bump_write_str (i32.const 859))
      )
    )
    (i32.eq (local.get $component_id) (global.get $component_b))
    (if 
      (then
        (call $bump_write_str (i32.const 874))
      )
    )
    (call $bump_write_str (i32.const 889))
    (loop $Stops
      (call $do_linear_gradient_stop_for (f32.div (local.get $i) (f32.const 4.0)) (local.get $component_id))
      drop
      (f32.add (local.get $i) (f32.const 1.0))
      (local.set $i)
      (f32.le (local.get $i) (f32.const 4.0))
      br_if $Stops
    )
    (call $bump_write_str (i32.const 938))
    (call $bump_write_done)
  )
  ...
  (func $do_css_color_lab_srgb (param $l f32) (param $a f32) (param $b f32) (result i32)
    ...
    (local.set $blue)
    (call $bump_write_start)
    (call $bump_write_str (i32.const 996))
    ...
    (global.set $bump_offset)
    (call $bump_write_str (i32.const 1004))
    (call $bump_write_done)
  )
)
